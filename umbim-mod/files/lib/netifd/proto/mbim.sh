...
_proto_mbim_setup() {
	...
	local device apn pincode delay pdp dhcp $PROTO_DEFAULT_OPTIONS
	local ip4addr ip4mask ip4gw ip4dns
	json_get_vars device apn pincode delay pdp dhcp auth username password $PROTO_DEFAULT_OPTIONS
...
#	mbim $DBG -n -t $tid -d $device pinstate || {
...
#	tid=$((tid + 1))
	...
	echo "$CONFIG" > /tmp/ip
	ip4addr=$(cat /tmp/ip | awk -F [\ /] '/ipv4address/{print $4}')
	ip4mask=$(cat /tmp/ip | awk -F [\ /] '/ipv4address/{print $5}')
	ip4gw=$(cat /tmp/ip | awk '/ipv4gateway/{print $2}')
	ip4dns=$(echo $(cat /tmp/ip | awk '/ipv4dnsserver/{print $2}'))

	uci_set_state network $interface tid "$tid"
	[ -n $pdp ] || pdp=ip
	[ -n $dhcp ] || dhcp=0
	echo "mbim[$$]" "Connected, set IPv4: ${ip4addr}/${ip4mask}"
	echo "mbim[$$]" "PDP is: $pdp"
